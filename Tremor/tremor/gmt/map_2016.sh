...
output="mendocino_march2016.ps"
resampling="0.004"  # smaller means more fine sampling (bigger file)

# Make color scale
gmt makecpt -T0/90/3 -Cjet -Z > myrainbow.cpt
...
awk '{print $1, $2, $3}' ../Dec08.txt | gmt psxy -R$range -J$projection -Sc0.20 -Cmyrainbow.cpt -K -O -P >> $output  
...
gmt psscale -R -J -DjTR+w3.4c/0.5c+o-1.5/-0.3 -Cmyrainbow.cpt -B20.0:"Days":/:: -P -O -K >> $output
