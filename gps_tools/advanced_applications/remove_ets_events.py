...
def view_single_station(station_name, offsets_remove=1, earthquakes_remove=0, outliers_remove=0, seasonals_remove=0,
    ...
    MyParams = configure(station_name, offsets_remove, earthquakes_remove, outliers_remove, seasonals_remove,
                         seasonals_type, datasource, refframe);
...
def configure(station, offsets_remove, earthquakes_remove, outliers_remove, seasonals_remove, seasonals_type,
    ...
    MyParams = Parameters(station=station, outliers_remove=outliers_remove, outliers_def=outliers_def,
                          ...
                          datasource=datasource, refframe=refframe, data_config=data_config_file);
    print("------- %s --------" % station);
    print(
        "Viewing station %s, earthquakes_remove=%d, outliers_remove=%d, seasonals_remove=%d, datasource=%s, "
        "refframe=%s" % (station, earthquakes_remove, outliers_remove, seasonals_remove, datasource, refframe));
    return MyParams;
...
def compute(myData, offset_obj, eq_obj, MyParams, ets_intervals):
    ...
    trend_out_uncorrected = gps_seasonal_removals.make_detrended_ts(newData, MyParams.seasonals_remove,
                                                                    MyParams.seasonals_type, MyParams.data_config);
    ...
    trend_out_corrected = gps_seasonal_removals.make_detrended_ts(ETS_removed, MyParams.seasonals_remove,
                                                                  MyParams.seasonals_type, MyParams.data_config);
...
def remove_characteristic_ETS(ts_obj, ets_intervals):
    ...
    for i in range(len(ts_obj.dtarray)):
        ...
        for j in range(len(ets_intervals)):
            if ets_intervals[j][0] <= ts_obj.dtarray[i] <= ets_intervals[j][1]:
                report = False;
        if report and ts_obj.dtarray[i] >= dt.datetime.strptime("20120101", "%Y%m%d"):
    ...
    ts_obj_gaps = Timeseries(name=ts_obj.name, coords=ts_obj.coords, dtarray=dtarray, dE=dE, dN=dN, dU=dU, Se=Se, Sn=Sn,
                             Su=Su, EQtimes=ts_obj.EQtimes);

    # Find the offsets associated with each ETS interval
    # Task: Get the east and north and up offsets here.
    offset_string = subprocess.check_output("grep " + ts_obj.name + " ../../GPS_POS_DATA/Tremor/Offsets_2mm_30days.txt",
                                            shell=True);
...
def remove_ETS_times(ts_obj, ets_intervals, offset_num_days):
    ...
    # ofile.write("%s %f %f %.2f %.2f %.2f %.2f %.2f %.2f\n" % (ts_obj.name, ts_obj.coords[0], ts_obj.coords[1],
    #                                                           np.mean(e_offsets), np.mean(n_offsets),
    #                                                           np.mean(u_offsets), np.std(e_offsets),
    #                                                           np.std(n_offsets), np.std(u_offsets) ) );
    # ofile.close();
...
# -------------- OUTPUTS ------------ #
def single_ts_plot(ts_obj, detrended, _MyParams, ets_intervals, name):
    label_fontsize = 18;
    ...
    # The major figure
    dpival = 500;
    [_f, axarr] = plt.subplots(3, 1, sharex=True, figsize=(10, 7), dpi=dpival);
