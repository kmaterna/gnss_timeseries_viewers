...
def compute(myVelfield, MyParams):
	...
	outfile.close();
	subprocess.call("gmt gpsgridder tempgps.txt -R"+MyParams.map_range+" -I"+str(MyParams.grid_inc)+" -S0.5 -Ff0.1 -C0.005 -Emisfitfile.txt -fg -r -Gnc4_%s.nc",shell=True);  # makes a netcdf grid file
	...
	[xdata, ydata, udata] = netcdf_io_functions.read_grd_xyz(file1,'lon','lat', 'z');
	...
	I2nd=np.zeros(np.shape(vdata));  # 2nd invariant of strain rate tensor
	max_shear=np.zeros(np.shape(vdata));  # max shear of strain rate tensor
	...
	for j in range(ydim-1):
		for i in range(xdim-1):
			...
			eyy=dvdy*1000;
			rot[j][i]=1000*0.25*(dvdx-dudy)*(dvdx-dudy)
