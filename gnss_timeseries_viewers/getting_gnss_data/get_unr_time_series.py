...
def configure(argv):
    ...
    if len(argv) != 2:
        ...
        unr_coordfile = argv[1]
    ref_frame1 = "NA"
    ref_frame2 = "IGS14"
    latlon_box = [-125.0, -110, 32.0, 49.0]  # A LARGE BOX INCLUDING ALL OF WUS
    return unr_coordfile, ref_frame1, ref_frame2, latlon_box


def get_stations(unr_coordfile, latlon_box):
    station_names = []
    data = np.genfromtxt(unr_coordfile, skip_header=2, usecols=(0, 1, 2), dtype=None)
    for item in data:
...
if __name__ == "__main__":
    coordfile, refframe1, refframe2, bbox = configure(sys.argv)
    station_list = get_stations(coordfile, bbox)
    download_stations(station_list, refframe1)  # NA first
    download_stations(station_list, refframe2)  # IGS14 next
