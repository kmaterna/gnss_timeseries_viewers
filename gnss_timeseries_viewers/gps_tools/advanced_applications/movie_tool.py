...
def turn_into_movie_ts(myobjects):
    ...
    for i in range(len(myobjects)):
        ...
        [dt_ds, dE_ds, dN_ds, dU_ds, Se_ds, Sn_ds, Su_ds] = interval_downsample(myobject.dtarray, myobject.dE,
                                                                                myobject.dN, myobject.dU,
                                                                                desired_downsample)
        downsampled_object = Timeseries(name=myobject.name, coords=myobject.coords, dtarray=dt_ds, dE=dE_ds, dN=dN_ds,
                                        dU=dU_ds, Se=Se_ds, Sn=Sn_ds, Su=Su_ds, EQtimes=myobject.EQtimes)
...
def interval_downsample(dtarray, dE, dN, dU, dt_downsample):
    ...
    dt_downsample_keep, dE_downsample, dN_downsample, dU_downsample = [], [], [], []
    Se_downsample, Sn_downsample, Su_downsample = [], [], []
    ...
    return [dt_downsample_keep, dE_downsample, dN_downsample, dU_downsample, Se_downsample, Sn_downsample,
            Su_downsample]
...
def write_outputs(objects, myparams):
    ...
    for i in range(len(objects)):
        for j in range(len(objects[i].dtarray)):
            ...
            ofile.write("%s %f %f %s %f %f %f \n" % (objects[i].name, objects[i].coords[0],
                                                     objects[i].coords[1], datestring, objects[i].dE[j],
                                                     objects[i].dN[j], objects[i].dU[j]))
    ofile.close()
