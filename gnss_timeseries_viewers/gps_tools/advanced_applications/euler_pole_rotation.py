...
def compute_euler_pole_rotation(vels, euler_pole):
    ...
    new_vels = Velfield(name=vels.name, nlat=vels.nlat, elon=vels.elon, n=new_north, e=new_east, u=new_up, sn=vels.sn,
                        se=vels.sn, su=vels.su, first_epoch=vels.first_epoch, last_epoch=vels.last_epoch)
    return new_vels
...
if __name__ == "__main__":
    ...
    [vels] = io_nota.read_pbo_vel_file(input_file)
    new_vels = compute_euler_pole_rotation(vels, euler_pole)
    # gps_tools.file_io.io_other.write_humanread_vel_file(new_vels, output_file)
    io_other.write_gmt_velfile(new_vels, output_file)
