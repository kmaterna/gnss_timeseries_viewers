...
def remove_seasonals_by_notch(Data):
    ...
    trended = Timeseries(name=Data.name, coords=Data.coords, dtarray=Data.dtarray, dN=dN_trended, dE=dE_trended,
                         dU=dU_trended, Sn=Data.Sn, Se=Data.Se, Su=Data.Su, EQtimes=Data.EQtimes);
...
def remove_seasonals_by_STL(Data, STL_dir):
    ...
    # ELSE: WE NEED TO RECOMPUTE
        ...
        # Return data
        Data = Timeseries(name=Data.name, coords=Data.coords, dtarray=final_dtarray, dN=final_dN, dE=final_dE,
                          dU=final_dU, Sn=final_Sn, Se=final_Se, Su=final_Su, EQtimes=Data.EQtimes);
...
def remove_seasonals_by_hydro(Data, hydro_dir, scaling=False):
    ...
    if scaling == True:
        ...
        if hydro_amp == 0.0:
            ...
            placeholder = np.full_like(Data.dtarray, np.nan, dtype=np.double)
            wimpyObj = Timeseries(name=Data.name, coords=Data.coords, dtarray=Data.dtarray, dN=placeholder,
