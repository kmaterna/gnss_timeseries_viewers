...
def read_real_gps_data():
	...
	newData=gps_ts_functions.remove_outliers(newData, 15);  # 10 mm outlier
...
start_index=3;
num_points=[]; formal_slope_uncertainty=[]; curvefit_slope_uncertainty=[];
...
plt.plot(num_points, ytest, '--g',label='n^(-3/2)');
plt.gca().set_yscale('log');
plt.gca().set_xscale('log');
plt.grid(True);
plt.legend();
plt.savefig('formal_uncertainty');
