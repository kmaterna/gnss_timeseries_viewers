...
gmt pscoast -R$range -J$projection -Slightblue -N1 -N2 -B1.0WeSn:."Mean Kernel": -Dh -K > $output1 # the title goes here
...
awk '{print $1, $2, $3, $4, $7, $8, $10}' $gpsfile | gmt psvelo -R$range -J$projection -O -K -Se$horiz_scale/0.68/8 -A+e+gblack+pthickest -Wthin,black >> $output1
...
gmt psscale -R$range -J$projection -DjTR+w12c/0.5c+o-1.5/0.2 -G0/15 -Cdatacpt2.cpt -B5.0:"":/:mm: -P -O >> $output1
